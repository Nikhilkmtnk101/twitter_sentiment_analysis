...
class SentimentAnalysis:
    ...
    def DownloadData(self, keyword, tweets):
        ...
        # authenticating
        bearer_token = 'AAAAAAAAAAAAAAAAAAAAAJyWywEAAAAArCs9wjfMjPgFPBrYuUNx%2BQU3gLU%3DM51WneAADWG47SEMObfxmyGeRE80INbmAo1wzVR2ICthE9xVl3'
        ...
        self.tweets = client.search_all_tweets(query=keyword, max_results=min(tweets, 100))
        # nitter_url = f"https://nitter.net/search?f=tweets&q={keyword}&since=&until=&near=&within=&lang=en"
        # headers = {"User-Agent": "Mozilla/5.0"}

        try:
            response = requests.get(nitter_url, headers=headers, timeout=10)
            if response.status_code != 200:
                print("Failed to fetch data from Nitter instance.")
                return

            soup = BeautifulSoup(response.text, "html.parser")
            tweets = soup.find_all("p", class_="tweet-content")

            # Extract up to tweet_count tweets
            self.tweets = [tweet.get_text().strip() for tweet in tweets[:tweets]]
